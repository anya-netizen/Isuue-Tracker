...
// Import components
import ConveyorBeltSystem from '../pg/ConveyorBeltSystem';
import CPOCapturePanel from '../pg/CPOCapturePanel';
import ClaimsWorkflow from '../pg/ClaimsWorkflow';
import PGPatientDashboard from '../pg/PGPatientDashboard';

export default function ServicesTab({ selectedPG, patients: initialPatients, pgs, tutorialData, onTutorialComplete }) {
  ...
  // Update patients when prop changes
  useEffect(() => {
    setPatients(initialPatients);
  }, [initialPatients]);

  // Check for patient selection from network mode
  useEffect(() => {
    const selectedPatientFromNetwork = sessionStorage.getItem('selectedPatientFromNetwork');
    if (selectedPatientFromNetwork) {
      try {
        const patient = JSON.parse(selectedPatientFromNetwork);
        setSelectedPatient(patient);
        setActiveTab('patient');
        // Clear the session storage
        sessionStorage.removeItem('selectedPatientFromNetwork');
      } catch (error) {
        console.error('Error parsing selected patient from network:', error);
        sessionStorage.removeItem('selectedPatientFromNetwork');
      }
    }
  }, []);
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      setLoading(true);
      try {
        const documentsData = await Document.list('-date_received');
  ...
  const handleRefreshData = useCallback(async () => {
    setRefreshing(true);
    try {
      const [patientsData, documentsData] = await Promise.all([
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900">
                    {tutorialData.attentionLevel === 'critical' ? 'Critical Action Required' :
                     tutorialData.attentionLevel === 'needs-attention' ? 'Performance Improvement Needed' :
                     'Maintain Good Performance'}
                  </h3>
                  <p className="text-sm text-gray-600">
                    Action requested for: <span className="font-semibold">{tutorialData.pgName}</span>
                  </p>
                </div>
              </div>
              <Button 
            ...
            <div className="space-y-4">
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  ...
                  <ul className="space-y-2 text-sm text-red-700">
                    <li className="flex items-start gap-2">
                      ...
                      <span><strong>Review unbillable patients:</strong> Check patient eligibility and documentation requirements</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-red-500 mt-1">•</span>
                      <span><strong>Update care coordination:</strong> Ensure all services are properly documented</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-red-500 mt-1">•</span>
                      <span><strong>Revenue recovery:</strong> Identify missed billing opportunities in recent encounters</span>
                    </li>
                  </ul>
                </div>
              )}

              {tutorialData.attentionLevel === 'needs-attention' && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <h4 className="font-semibold text-yellow-800 mb-2">📋 Improvement Opportunities:</h4>
                  <ul className="space-y-2 text-sm text-yellow-700">
                    <li className="flex items-start gap-2">
                      <span className="text-yellow-500 mt-1">•</span>
                      <span><strong>Optimize patient flow:</strong> Review scheduling and appointment management</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-yellow-500 mt-1">•</span>
                      <span><strong>Enhance documentation:</strong> Improve coding accuracy and completeness</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-yellow-500 mt-1">•</span>
                      <span><strong>Monitor metrics:</strong> Set up alerts for key performance indicators</span>
                    </li>
                  </ul>
                </div>
              )}

              {tutorialData.attentionLevel === 'good' && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <h4 className="font-semibold text-green-800 mb-2">✅ Maintain Excellence:</h4>
                  <ul className="space-y-2 text-sm text-green-700">
                    <li className="flex items-start gap-2">
                      <span className="text-green-500 mt-1">•</span>
                      <span><strong>Continue best practices:</strong> Document what's working well for replication</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-green-500 mt-1">•</span>
                      <span><strong>Share knowledge:</strong> Help other PGs improve using your successful strategies</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <span className="text-green-500 mt-1">•</span>
                      <span><strong>Monitor trends:</strong> Watch for early signs of performance changes</span>
                    </li>
                  </ul>
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                <Badge variant="outline" className="text-xs">
                  Action initiated from Map
                </Badge>
                <Badge variant="outline" className="text-xs">
                  {new Date(tutorialData.timestamp).toLocaleTimeString()}
                </Badge>
              </div>
            </div>
          </CardContent>
      ...
      {selectedPG ? (
        <>
          ...
          <Card className="shadow-2xl border-0 bg-white/90 backdrop-blur-xl">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="grid w-full grid-cols-4 bg-gradient-to-r from-gray-50 to-gray-100 p-2 rounded-2xl shadow-inner">
        ...
        >
          <Card className="shadow-xl bg-white/80 backdrop-blur-sm p-8 border-0">
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">Welcome to Services Dashboard</h3>
            <p className="text-gray-500 mb-6">Please select a Healthcare Service Group to access comprehensive management tools.</p>
          </Card>
        </motion.div>
      )}
    </div>
